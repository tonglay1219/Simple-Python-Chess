...
class BoardGuiTk(Frame):
    ...
    def __init__(self, parent, chessboard, square_size=64):
        ...
        self.canvas = Canvas(self, width=canvas_width, height=canvas_height, background="grey")
        self.canvas.pack(side="top", fill="both", anchor="c", expand=True)

        self.canvas.bind("<Configure>", self.refresh)
        self.canvas.bind("<Button-1>", self.click)

        self.statusbar = Frame(self, height=64)
        ...
        self.button_save = Button(self.statusbar, text="Save", fg="black", command=self.chessboard.save_to_file)
        self.button_save.pack(side=LEFT, in_=self.statusbar)

        self.label_status = Label(self.statusbar, text="   White's turn  ", fg="black")
        self.label_status.pack(side=LEFT, expand=0, in_=self.statusbar)

        self.button_quit = Button(self.statusbar, text="Quit", fg="black", command=self.parent.destroy)
    ...
    def draw_pieces(self):
        ...
        for coord, piece in self.chessboard.iteritems():
            ...
            if piece is not None:
                filename = os.path.dirname(os.path.abspath(__file__)).replace("/chesslib", "/") + "img/%s%s.png" % (
                piece.color, piece.abbriviation.lower())
                ...
                # self.addpiece(piecename, filename, x, y)
...
def display(chessboard):
    app = welcome_window.FrameHelper()
    app.mainloop()
